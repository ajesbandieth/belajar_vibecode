...
<aside id="sidebar" class="fixed top-16 left-0 bottom-0 w-64 bg-white border-r border-slate-200 py-6 overflow-y-auto z-30 -translate-x-full lg:translate-x-0">
    <nav class="space-y-1">
        <a href="<?php echo base_url('dashboard'); ?>" class="sidebar-item <?php echo (isset($active_menu) && $active_menu == 'dashboard') ? 'active' : ''; ?> flex items-center gap-3 px-6 py-3 font-medium transition-all">
            <i class="fas fa-home w-5"></i> <span class="sidebar-text">Dashboard</span>
        </a>
        <a href="<?php echo base_url('categories'); ?>" class="sidebar-item <?php echo (isset($active_menu) && $active_menu == 'categories') ? 'active' : ''; ?> flex items-center gap-3 px-6 py-3 font-medium text-slate-600 transition-all">
            <i class="fas fa-folder w-5"></i> <span class="sidebar-text">Manage Categories</span>
        </a>
        <a href="<?php echo base_url('photos'); ?>" class="sidebar-item flex items-center gap-3 px-6 py-3 font-medium <?php echo (isset($active_menu) && $active_menu == 'photos') ? 'active' : 'text-slate-600'; ?> transition-all">
            <i class="fas fa-image w-5"></i> <span class="sidebar-text">My Photos</span>
        </a>
        <a href="<?php echo base_url('videos'); ?>" class="sidebar-item flex items-center gap-3 px-6 py-3 font-medium <?php echo (isset($active_menu) && $active_menu == 'videos') ? 'active' : 'text-slate-600'; ?> transition-all">
